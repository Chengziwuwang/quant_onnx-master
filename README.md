...
-导出模型大小约13.6GB：
qwen3_fp32.onnx约1.45MB
外部权重文件总大小约13.6GB
-量化后模型大小约为2.76GB，包括：
qwen3_int8.onnx约2.45MB
外部权重文件总大小约2.76GB
-压缩比约为20%

效果观察：
1. 量化后的模型在 CPU 上生成文字的速度如何？（主观感受即可，如：每秒大概几个字？）
生成速度约2-5tokens/秒。生成一句话（约 20 字）需要 6-10 秒：
-首token延迟：约0.5-1秒
-后续token生成：每个token约0.2-0.5秒
1. 量化后的模型是否依然能生成通顺的中文？请在报告中附上 2-3 轮对话的截图。
量化后的模型能生成通顺的中文，如图所示3轮对话效果截图：
![对话效果1](image/20260119224702.png)

## 思考题：
在任务三中，目前的推理逻辑每生成一个字都要重新计算整个序列。如果想要加速，应该引入什么机制？（提示：KV Cache）
引入KV Cache机制，通过缓存历史K、V数据消除重复计算，实现推理加速。
当前实现中，每次生成新token时，都需要将整个序列重新输入模型进行前向传播，这会导致：存在大量重复计算，计算复杂度为O(n²)，其中n是序列长度，推理速度随序列长度增长而显著下降。
KV Cache机制的基本原理：
在Transformer的自注意力机制中，每个token的Key和Value向量在生成过程中保持不变，只需要缓存之前所有token的K和V，新token生成时只需计算当前token的Q，然后与缓存的K、V进行attention计算
```python
   #伪代码示例
   kv_cache = {}  #存储历史token的K和V
   for step in range(max_tokens):
       ...
       if step == 0:
           ...
           #只计算新token的K和V，并更新缓存
           new_k, new_v = model.compute_kv([next_token])
           kv_cache['k'] = concat([kv_cache['k'], new_k])
           kv_cache['v'] = concat([kv_cache['v'], new_v])
       #使用缓存的K和V进行attention计算
       logits = model.attention_with_cache(q, kv_cache['k'], kv_cache['v'])
   ```
